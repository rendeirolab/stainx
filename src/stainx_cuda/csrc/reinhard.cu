...
torch::Tensor reinhard_cuda(torch::Tensor input_images, torch::Tensor reference_mean, torch::Tensor reference_std) {
    ...
    if (err != cudaSuccess) {
        TORCH_CHECK(false, "CUDA error in rgb_to_lab_kernel: ", cudaGetErrorString(err),
                   ". Input shape: (", N, ", ", C, ", ", H, ", ", W, ")");
    }
    ...
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        TORCH_CHECK(false, "CUDA error in statistics_matching_kernel: ", cudaGetErrorString(err),
                   ". Input shape: (", N, ", ", C, ", ", H, ", ", W, ")");
    }
    ...
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        TORCH_CHECK(false, "CUDA error in lab_to_rgb_kernel: ", cudaGetErrorString(err),
                   ". Input shape: (", N, ", ", C, ", ", H, ", ", W, ")");
    }
