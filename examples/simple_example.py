...
def add_rounded_corners_pil(image_array, radius=75, border_thickness=100):
    ...
    if radius > max_radius:
        print(f"Warning: radius {radius} exceeds maximum {max_radius} for image size {pil_img.width}x{pil_img.height}. Clamping to {max_radius}.")
        radius = max_radius
    elif radius > practical_max_radius:
        print(f"Note: radius {radius} is large. Consider using <= {practical_max_radius} for better visual results.")
    ...
    return np.array(final).astype(np.float32) / 255.0
...
def main():
    ...
    data_dir = os.path.join(os.path.dirname(__file__), "data") if args.data_dir is None else args.data_dir
    ...
    if args.visualize or args.save_plots:
        ...
        visualize_results(reference_cpu, source_cpu, result_cpu, test_paths, args.method, save_plots=args.save_plots, output_dir=args.output_dir)
