...
__global__ void hm_partial_hist_kernel(const uint8_t* images_uint8, uint32_t* partial_hist, int N, int C, int H, int W, int blocks_per_channel) {
    ...
    for (int i = threadIdx.x; i < HM_NUM_BINS; i += blockDim.x) { hist_s[i] = 0; }
    __syncthreads();
    ...
    for (int i = threadIdx.x; i < HM_NUM_BINS; i += blockDim.x) { partial_hist[base + i] = hist_s[i]; }
}
...
__global__ void hm_reduce_hist_kernel(const uint32_t* partial_hist, uint32_t* hist_out, int C, int blocks_per_channel) {
    ...
    for (int b = 0; b < blocks_per_channel; ++b) { sum += partial_hist[base_channel + b * HM_NUM_BINS + bin]; }
...
#pragma unroll
    ...
    if (bin < HM_NUM_BINS) { hist_s[bin] = hist_s[bin] / (sum_s + 1e-8f); }
    __syncthreads();
...
#pragma unroll
    ...
    if (bin < HM_NUM_BINS) { ref_cdf[channel_idx * HM_NUM_BINS + bin] = cdf_s[bin]; }
}
...
__global__ void hm_build_lut_kernel(const uint32_t* hist_u32, const float* ref_cdf, float* lut, int N, int H, int W) {
    ...
    if (bin < HM_NUM_BINS) { ref_cdf_s[bin] = ref_cdf[channel_idx * HM_NUM_BINS + bin]; }
    __syncthreads();
